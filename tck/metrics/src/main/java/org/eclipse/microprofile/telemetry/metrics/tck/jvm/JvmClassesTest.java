...
public class JvmClassesTest extends Arquillian {
    ...
    @Test
    void testClassLoadedMetrics() {
        MetricData metric = metricExporter.getMetricData("jvm.class.loaded").get(0);
        Assert.assertEquals(metric.getDescription(), "Number of classes loaded since JVM start.");
        Assert.assertEquals(metric.getType(), MetricDataType.LONG_SUM);
        Assert.assertEquals(metric.getUnit(), "{class}");
    }

    @Test
    void testClassUnloadedMetrics() {
        MetricData metric = metricExporter.getMetricData("jvm.class.unloaded").get(0);
        Assert.assertEquals(metric.getDescription(), "Number of classes unloaded since JVM start.");
        Assert.assertEquals(metric.getType(), MetricDataType.LONG_SUM);
        Assert.assertEquals(metric.getUnit(), "{class}");
    }
    ...
    void testClassCountMetrics() {
        MetricData metric = metricExporter.getMetricData("jvm.class.count").get(0);
        Assert.assertEquals(metric.getDescription(), "Number of classes currently loaded.");
        Assert.assertEquals(metric.getType(), MetricDataType.LONG_SUM);
        Assert.assertEquals(metric.getUnit(), "{class}");
    }
