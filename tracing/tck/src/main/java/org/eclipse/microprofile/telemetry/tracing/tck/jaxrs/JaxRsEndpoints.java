 ...
 * <p>
 * Whenever a request is made or received by one of these features, MP Telemetry should create a span. Trace ID
 * information should be sent via the HTTP headers so that the traces from both the client and server can be joined up.
 * Baggage information should also be sent via the HTTP headers so that contextual information from the first service
 * can be included in trace messages in the second service.
 * <p>
 * This class covers:
 * <p>
 ...
 * <ul>
 * <li>TCK RestClientSpanTest only checks HTTP_STATUS, HTTP_METHOD, HTTP_SCHEME, HTTP_TARGET, HTTP_URL, see if there are
 * any others which should be present
 * <li>There are constants defined for each of the attributes in the spec
 * </ul>
 * <p>
 * <strong>NOTE</strong>: Some of the tests in this class could be re-written more simply using {@code FATServlet} and
 * {@code TestSpans}. However, we've sometimes seen slightly different behaviour for JAX-RS client and MP Rest client
 * depending on whether or not they're called via a JAX-RS resource method. Therefore we're leaving these tests as they
 * are so that we have some test coverage of calling JAX-RS client via a JAX-RS resource method.
 */
@ApplicationPath("")
@Path("endpoints")
public class JaxRsEndpoints extends Application {
    ...
    public Response readSpans(@Context UriInfo uriInfo, @PathParam("traceId") String traceId) {
        ...
        assertTrue(httpGet.getAttributes().get(HTTP_URL).contains("endpoints")); // There are many different URLs
        ...
        assertTrue(httpGet.getAttributes().get(HTTP_URL).contains("endpoints"));
